...
Attributes.prototype.stripProperties = function(attributes) {
  ...
  for(var attribute in attributes) {
    if(_.isObject(attributes[attribute])) {
      // delete hole object, if attribute isn't in the reserved words registry
      if (utils.reservedWords.indexOf(attribute) === -1) {
        delete attributes[attribute];
      }
      this.stripProperty(attributes[attribute]);
    }
    else if(utils.reservedWords.indexOf(attribute) === -1) { // in case it's not object. E.g. {myAttr: "string"}
      delete attributes[attribute];
    }
  }
