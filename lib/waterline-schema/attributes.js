...
Attributes.prototype.stripProperties = function(attributes) {   // edited method manually

  for(var attribute in attributes) {
    if(_.isObject(attributes[attribute])) {
      this.stripProperty(attributes[attribute]);
    }
    // otherwise, it will fail. E.g. {myAttr: "string"}, "string" will be passed, instead of object
    else if(_.isString(attributes[attribute])) {
      var objAttritbute = {};
      objAttritbute[attribute] = attributes[attribute];
      this.stripProperty(objAttritbute);
    }
  }
