    ...
    _.each(collection.schema, function processCollectionAttribute(attributeVal, attributeName) {
      ...
      _.each(joinTables, function lookThroughJoinTables(table) {
        ...
        if (_.indexOf(tablesInUse, collectionName) > -1 || _.indexOf(tablesInUse, attributeVal.collection) > -1) {
          ...
          var column = attributeVal.collection.toLowerCase() + '_' + attributeVal.via;
      ...
      if (!_.has(tableCollection.attributes, collectionName.toLowerCase() + '_' + attributeName)) {
        return;
      }
      ...
      collection.schema[attributeName] = {
        ...
        on: tableCollection.attributes[collectionName.toLowerCase() + '_' + attributeName].columnName,
        onKey: tableCollection.attributes[collectionName.toLowerCase() + '_' + attributeName].columnName
      };
    });
  });
};
