...
module.exports = function flagThroughTable(collectionName, schema) {
  var attributes = schema[collectionName.toLowerCase()].schema;
  ...
  _.each(attributes, function lookForThroughProperty(attributeVal, attributeName) {
    ...
    schema[linkedCollectionName.toLowerCase()].throughTable = schema[linkedCollectionName.toLowerCase()].throughTable || {};
    var throughPath = collectionName + '.' + attributeName;
    var linkedAttrs = schema[linkedCollectionName.toLowerCase()].schema;

    // Set the schema value on the through table
    schema[linkedCollectionName.toLowerCase()].hasSchema = true;

    // Find the attribute on the linked collection that references this collection
    schema[linkedCollectionName.toLowerCase()].throughTable[throughPath] =  _.find(_.keys(linkedAttrs), function checkReference(attrName) {
      if (!linkedAttrs[attrName].referenceIdentity) {
        return false;
      }

      return linkedAttrs[attrName].referenceIdentity.toLowerCase() === attributeVal.collection.toLowerCase();
    });

    // Build up proper reference on the attribute
    attributeVal.references = schema[linkedCollectionName.toLowerCase()].tableName;
    ...
    _.each(linkedAttrs, function lookForLinkedAttribute(linkedAttrVal, linkedAttrName) {
      ...
      if (linkedAttrVal.references !== schema[collectionName.toLowerCase()].tableName) {
        return;
      }
