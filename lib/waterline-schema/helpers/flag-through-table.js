...
module.exports = function flagThroughTable(collectionName, schema) {
  ...
  _.each(attributes, function lookForThroughProperty(attributeVal, attributeName) {
    ...
    if (!_.any(schema[oppositeCollectionName].schema, function(oppositeAttrVal) {
      ...
      throw flaverr({ name: 'userError', code: 'E_MISSING_OPPOSITE_ASSOC'}, new Error('A `through` property was set on the attribute `' + attributeName + '` on the `' + collectionName + '` model but no corresponding attribute in the `' + oppositeCollectionName + '` model could be found pointing with the same `through` value (`' + linkedCollectionName + '`).'));
    }
    ...
    if (!reference) {
      throw flaverr({ name: 'userError', code: 'E_MISSING_OPPOSITE_ASSOC'}, new Error('A `through` property was set on the attribute `' + attributeName + '` on the `' + collectionName + '` model but no reference in the through model was found.'));
    }
