...
module.exports = function flagThroughTable(collectionName, schema) {
  ...
  _.each(attributes, function(attributeVal, attributeName) {
    ...
    var linkedCollectionName = attributeVal.through.toLowerCase();
    ...
    // Flag the linkedCollection as a junction table
    schema[linkedCollectionName].junctionTable = true;
    schema[linkedCollectionName].hasSchema = true;

    // Find the attribute on the linked collection that references this collection
    schema[linkedCollectionName].throughTable[throughPath] = _.find(_.keys(linkedAttrs), function(attrName) {
