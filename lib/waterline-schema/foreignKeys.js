...
module.exports = function foreignKeysMapper(schema) {
  ...
  _.each(schema, function parseForeignKeys(collection, collectionName) {
    //  ╦═╗╔═╗╔═╗╦  ╔═╗╔═╗╔═╗  ┌┬┐┌─┐┌┬┐┌─┐┬
    //  ╠╦╝║╣ ╠═╝║  ╠═╣║  ║╣   ││││ │ ││├┤ │
    //  ╩╚═╚═╝╩  ╩═╝╩ ╩╚═╝╚═╝  ┴ ┴└─┘─┴┘└─┘┴─┘
    //  ┌─┐┌─┐┌─┐┌─┐┌─┐┬┌─┐┌┬┐┬┌─┐┌┐┌┌─┐
    ...
    _.each(collection.schema, function(attributeVal, attributeName) {
      ...
      var relatedModelName = attributeVal.model.toLowerCase();
      ...
      var foreignKey = {
        ...
        foreignKey: true,
        references: relatedModelName.toLowerCase(),
