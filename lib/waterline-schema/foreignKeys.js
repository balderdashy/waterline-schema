...
ForeignKeys.prototype.replaceKeys = function(attributes) {
  ...
  for(var attribute in attributes) {
    ...
    var columnName = this.buildColumnName(attributes[attribute], primaryKey);
    var foreignKey = {
 ...
 * the linked table and the primary key field.
 *
...
ForeignKeys.prototype.buildColumnName = function(attribute, primaryKey) {
  if(attribute.hasOwnProperty('columnName')) return attribute.columnName.toLowerCase();
  return attribute.model.toLowerCase() + '_' + primaryKey.name.toLowerCase();
};
