...
module.exports = function buildJoinTables(schema) {
  ...
  _.each(schema, function parseCollectionAttributes(collection, collectionName) {
    ...
    _.each(collection.schema, function parseAttributes(attributeVal, attributeName) {
      ...
      var relatedModel = schema[relatedModelName.toLowerCase()];
      if (!relatedModel) {
      ...
      collection.schema[attributeName] = {
        ...
        on: joinTableSchema.attributes[collectionName + '_' + attributeName].columnName,
        onKey: joinTableSchema.attributes[collectionName + '_' + attributeName].columnName
      };
