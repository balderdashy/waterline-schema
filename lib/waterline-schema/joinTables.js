...
JoinTables.prototype.buildJoins = function(collection) {
  ...
  for(var attribute in collectionAttributes) {
    ...
    attr = collectionAttributes[attribute];
    ...
    if(!hop(child.attributes, attr.via)) {
      err = 'Collection ' + collection + ' has a `via` attribute which is missing from the collection ' + attr.collection;
      throw new Error(err);
    }

    // If the associated attribute is a foreign key, ignore it
    if(hop(child.attributes[attr.via], 'foreignKey')) continue;
...
JoinTables.prototype.searchForAttribute = function(collectionName, attributeName) {
  ...
  Object.keys(collection.attributes).forEach(function(key) {
    ...
    if(hop(properties, attributeName)) matching = key;
  });
