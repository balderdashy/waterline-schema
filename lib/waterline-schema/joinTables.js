...
JoinTables.prototype.buildJoins = function(collection) {
  ...
  for(var attribute in collectionAttributes) {
    ...
    // realtionship. If it's a one-to-many we don't need to build up a join table.
    ...
    if(hasOwnProperty(child.attributes[attr.via], 'model')) continue;
...
JoinTables.prototype.buildTable = function(columns) {
  ...
  table.attributes[c1.collection + '_' + c1.attribute] = this.buildForeignKey(c1);
  table.attributes[c2.collection + '_' + c2.attribute] = this.buildForeignKey(c2);
...
JoinTables.prototype.buildForeignKey = function(column) {
  var primaryKey = this.findPrimaryKey(column.collection);
  var columnName = (column.collection + '_' + column.attribute).toLowerCase();

  return {
    columnName: columnName,
    type: primaryKey.attributes.type,
    foreignKey: true,
    references: column.collection.toLowerCase(),
    on: primaryKey.name.toLowerCase(),
    via: column.collection + '_' + column.via,
    groupKey: column.collection.toLowerCase()
  };
};
