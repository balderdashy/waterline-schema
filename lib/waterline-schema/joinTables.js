...
module.exports = function buildJoinTables(schema) {
  ...
  _.each(schema, function parseCollectionAttributes(collection, collectionName) {
    ...
    //  ├─┤ │  │ ├┬┘│├┴┐│ │ │ ├┤ └─┐
    //  ┴ ┴ ┴  ┴ ┴└─┴└─┘└─┘ ┴ └─┘└─┘
    _.each(collection.schema, function(attributeVal, attributeName) {
      ...
      var relatedModelName = attributeVal.collection.toLowerCase();
      ...
      if(_.has(attributeVal, 'via') && _.has(relatedModel.schema[relatedModel.via], 'foreignKey')) {
        return;
      }
      ...
      var tableAttributes = {
        ...
        column_one: {
          collection: collection.identity.toLowerCase(),
      ...
      var joinTableSchema = buildJoinTableSchema(tableAttributes, schema);
      ...
      _.each(joinTables, function(table) {
  ...
  _.each(joinTables, function(table) {
    schema[table.identity.toLowerCase()] = {
      primaryKey: table.primaryKey,
      identity: table.identity,
      tableName: table.tableName || table.identity,
