...
module.exports = function mapReferences(schema) {
  _.each(schema, function(collection, collectionName) {
    _.each(collection.schema, function(attributeVal, attributeName) {
      ...
      if (!reference) {
        throw new Error('A `collection` property was set on the attribute ' + attributeName + ' on the ' + collectionName + ' model that points to ' + linkedCollection + ' but no reference was found matching that model.');
      }
