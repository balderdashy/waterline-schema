...
module.exports = function mapReferences(schema) {
  _.each(schema, function(collection, collectionName) {
    _.each(collection.schema, function(attributeVal, attributeName) {
      ...
      var linkedAttrs = schema[linkedCollection.toLowerCase()].schema;
      ...
      _.each(linkedAttrs, function(linkedAttrVal, linkedAttrName) {
        ...
        if(linkedAttrVal.references !== collectionName.toLowerCase()) {
          return;
        }

        if(via.toLowerCase() !== linkedAttrName.toLowerCase()) {
          return;
        }
