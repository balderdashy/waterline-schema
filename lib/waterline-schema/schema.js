  ...
  _.each(collections, function normalizeCollection(collectionPrototype) {
    ...
    _.each(collection.attributes, function(attribute, attributeName) {
      ...
      if (_.has(attribute, 'columnName')) {
        delete collection.attributes[attributeName].columnName;
      }
    }); // </ .each(attributes)
    ...
    _.each(collection.schema, function(attribute, attributeName) {
      if (!_.has(attribute, 'columnName')) {
        collection.schema[attributeName].columnName = attributeName.trim().replace(' ', '_');
      }
    }); // </ .each(schema)
  }); // </ .each(collections)
