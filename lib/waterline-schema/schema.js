    ...
    _.each(schemaCollection.attributes, function(attribute, attributeName) {
      ...
      if (_.has(attribute, 'defaultsTo')) {
        ...
        if (_.isNull(attribute.defaultsTo)) {
          throw new Error('The attribute ' + attributeName + ' contains a default value set to `null`. Using `null` as a default is not supported (a null value will be set automatically for missing attributes).');
        }

        // Verify that the attribute isn't required
        if (attribute.required) {
          throw new Error('The attribute ' + attributeName + ' contains both a `defaultsTo` value and is set to `required`. The attribute can have either property but not both.');
        }
      }
