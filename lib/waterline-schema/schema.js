  ...
  _.each(collections, function normalizeCollection(collectionPrototype) {
    ...
    _.each(schemaCollection.attributes, function parseAttributes(attribute, attributeName) {
      ...
      if (_.has(attribute, 'collection') && _.has(attribute, 'via')) {
        if (attribute.via === attributeName) {
      ...
      if (_.has(attribute, 'autoCreatedAt')) {
        ...
        if (attribute.type !== 'number' && attribute.type !== 'string') {
          throw new Error('The attribute `' + attributeName + '` on the `' + collection.identity + '` model contains both an `autoCreatedAt` flag but is not a proper type. Auto timestamps must have either a `number` or `string` type set.');
        }
      }
      ...
      if (_.has(attribute, 'autoUpdatedAt')) {
        ...
        if (attribute.type !== 'number' && attribute.type !== 'string') {
          throw new Error('The attribute `' + attributeName + '` on the `' + collection.identity + '` model contains both an `autoUpdatedAt` flag but is not a proper type. Auto timestamps must have either a `number` or `string` type set.');
        }
      }
